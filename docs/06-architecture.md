...
sequenceDiagram
  ...
  Note over Backend,DB: 짧게 연속된 메시지는 같은 발화 묶음에 포함

  par Backend 영속 작업 처리
    Worker->>DB: 대화별 다음 발화 묶음 선점
    Worker->>Agent: 묶음 단위 답변 생성 요청(turnId)
...
- 같은 대화에서 짧게 연속된 사용자 메시지는 하나의 발화 묶음으로 합쳐 한 번
  답변한다. 답변 생성 중 도착한 메시지는 다음 묶음으로 넘긴다.
- 같은 대화의 발화 묶음은 순서대로 하나씩 처리하고, 서로 다른 대화는 병렬
  처리한다. Agent 문맥에는 현재 묶음보다 뒤에 대기 중인 메시지를 포함하지 않는다.
...
- Agent 연결 실패, 일시 장애, timeout은 동일한 `turnId`로 최초 요청을 포함해
  ...
  번만 반영한다.
- 크레딧은 발화 묶음마다 한 번 예약한다. 묶음이 대기·생성·재시도 중인 동안
  예약을 유지하고, 실제 처리가 시작된 뒤 15분 안에 성공하면 캡처한다. 최종
  실패하거나 15분을 넘으면 예약을 해제한다.
- 최종 실패 시 사용자 메시지는 유지하고 발화 묶음을 `failed`로 기록하며,
  크레딧 예약을 해제한 뒤 다음 묶음을 계속 처리한다. 사용자에게는 내부 실패
  원인을 노출하지 않는다.
- 실패한 묶음은 사용자가 명시적으로 재시도할 수 있다. 재시도는 현재 대화의
  마지막 처리 순서에 배치하고 새 크레딧 예약과 최대 3회·15분 정책을 적용하며,
  중복 재시도 요청은 작업을 하나만 만든다.
