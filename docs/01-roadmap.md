...
- 현재 단계: **캐릭터 관계 기능**. (결정)
- 활성 목표: **DM 답변 생성 비동기 전환**. (결정)
  - 사용자 메시지 전송은 service-backend의 사용자 메시지와 영속 답변 작업 저장
    성공까지 동기 처리한다.
  - service-backend worker가 기존 opod-agent API를 요청-응답 방식으로 호출하고
    결과를 저장한다.
  - 같은 대화의 짧은 연속 메시지는 하나의 발화 묶음으로 합쳐 한 번 답하고,
    묶음은 대화별 순서대로 처리한다.
  - opod-web은 `GET /messages`의 cursor를 이용해 새 메시지를 주기적으로 조회한다.
  - 현재 구현은 `POST /messages`가 Agent 답변 완료까지 기다리므로 목표 구조와
    어긋난다. (gap)
...
| DM 답변 생성 비동기 전환                   | `messages.service.ts`가 답변 완료까지 동기 대기하고, service-backend가 소유하는 영속 답변 작업과 worker가 없음                                                                                                             |
