...
- 현재 단계: **캐릭터 관계 기능**. (결정)
- 활성 목표: **DM 답변 생성 비동기 전환**. service-backend 구현 완료(2026-08-11).
  - 사용자 메시지 전송은 사용자 메시지와 영속 답변 작업 저장 성공까지 동기
    처리하고 즉시 201을 준다.
  - `MessageReplyWorker`가 기존 opod-agent API를 요청-응답 방식으로 호출하고
    결과를 저장한다. 대화별 직렬, 대화 간 병렬.
  - opod-web은 `GET /messages`의 cursor를 이용해 새 메시지를 주기적으로 조회한다.
    각 메시지의 `replyStatus`가 pending·completed·failed를 알려준다.
  - 연속 발화를 하나로 묶어 한 번 답하는 동작은 범위에서 제외했다(2026-08-11).
    지금은 메시지 1건마다 답변 1건과 크레딧 1회다. (유보)
  - opod-web 폴링 UX와 pending·failed 표시는 미구현. (gap)
...
- 확장 아이디어:
  - service-backend와 opod-agent 사이의 답변 작업 전달을 메시지 브로커(MQ 또는
    Kafka)로 전환. 현재는 Backend 영속 작업 + Agent HTTP 요청-응답을 사용한다.
    (결정/유보)
  ...
  - pgvector 기반 하이브리드 컨텍스트 선별 — 트리거 도달 시 전환
    (근거: `db-management.md`).
- 의존:
  - 결제 provider 운영 전환 (Polar/Apple/Google adapter 구현, credential·상품 ID
    등록과 sandbox 검증 필요). (구현됨/운영 설정 유보)
...
| 2026-07-29 | 실시간 채팅 목표 = POST + SSE                                                                                                | 2026-08-10 결정으로 대체                                                                                |
...
| 2026-08-04 | 결제는 provider-neutral 경계로 분리하고 Web은 Polar, 앱은 Apple/Google 직접 검증                                             | PG 교체 가능성, backend-only 구현 결정                                                                  |
| 2026-07-29 | 성인인증 provider 방향 미정                                                                                                  | 현재 성인인증 미구성                                                                                    |
| 2026-07-29 | affinity 정책 = 초안 유지(미확정·미구현)                                                                                     | 참고 설계로만 보존                                                                                      |
| 2026-07-29 | 배포 = 자택 단일 서버, staging 미정                                                                                          | `deploy.sh`                                                                                             |
| 2026-08-06 | DB 환경 용어 = 로컬 / 개발 / 운영(아직 없음) / 테스트 4단어 고정                                                             | 용어 혼용이 파괴적 작업의 대상을 흐린다. 정본은 `02-development-rules.md`                               |
| 2026-08-06 | `deploy.sh` 배포 대상 = **개발 서버**(`dev-run-taeho`), 운영 환경은 아직 없음                                                | 기존 문서가 이를 "Production"이라 적은 것은 오기                                                        |
| 2026-08-10 | DM = 메시지·영속 작업 저장까지 동기, 연속 발화 묶음별 직렬 답변, Backend worker가 Agent 요청-응답 처리, Web은 cursor polling | 사람처럼 연속 발화를 한 번에 이해하면서 긴 HTTP 요청과 작업 유실을 피하고 기존 Agent·조회 계약을 재사용 |
| 2026-08-10 | Agent 일시 장애·timeout은 동일 turnId로 최대 3회 시도하고, 차후 Backend↔Agent 전송을 MQ 또는 Kafka로 교체                    | 현재는 직접 요청-응답으로 단순하게 구현하고 transport 교체는 유보                                       |
| 2026-08-10 | chat_reply 크레딧은 발화 묶음마다 한 번 예약하고 작업 상태에 연동하며, 처리 시작 후 15분에 최종 종료                         | 독립 TTL 경합과 무기한 예약 잠금을 함께 방지                                                            |
| 2026-08-10 | 최종 실패는 발화 묶음 failed + 예약 해제 후 다음 묶음을 처리하고, 명시적 재시도는 대화 마지막에 배치                         | 사용자 메시지와 대화 순서를 보존하면서 실패 복구를 허용                                                 |
| 2026-08-11 | 발화 묶음은 첫 구현 범위에서 제외 (유보)                                                                                     | 비동기 전환을 먼저 세운다. 대가는 연속 발화의 중복 답변·중복 차감. 되살릴 때 스키마 변경은 없다        |
| 2026-08-11 | 답변 worker = 새 의존성 없는 프로세스 내 주기 tick                                                                           | 재시도의 미래 준비시각과 크래시 후 lease 회수에 시간 기반 트리거가 필요                                 |
...
| DM 답변 생성 비동기 전환                   | service-backend는 해소됨(2026-08-11). 남은 gap은 opod-web — 폴링 주기, 화면 이탈 시 중단, pending·failed 표시가 아직 없어 답변이 오는 것을 화면이 보여주지 못한다                                                          |
