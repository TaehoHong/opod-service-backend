...
| messages       | `src/domain/messages`, `src/service/messages`            | DM 대화·전송(작업 저장까지 동기)·비동기 답변 worker·읽음/미읽음 | `MessagesController`, `MessagesService`, `MessageReplyWorker` | database, credits, events, characters, reply-provider | `messages.service.spec.ts`, `message-reply.worker.spec.ts`, `message-reply.provider.spec.ts`, `test/messages.e2e-spec.ts` | `src/domain/messages/messages.service.ts`, `message-reply.worker.ts`             |
| credits        | `src/domain/credits`, `src/service/credits`              | 불변 크레딧 원장·잔액·사용 출처·예약·회수·출석                  | `CreditsController`, `CreditsService`                      | database                                              | `credits.service.spec.ts`, `payment-refund-coverage.spec.ts`, `test/credits.e2e-spec.ts`   | `src/domain/credits/credits.service.ts`, `credit-pricing.ts`                     |
| purchases      | `src/domain/purchases`, `src/service/purchases`          | 크레딧 상품·구매·환불 유스케이스 조정                           | `PurchasesController`, `PurchasesService`                  | database, payments, credits, notifications            | `purchases.service.spec.ts`, `test/credits.e2e-spec.ts`                                    | `src/domain/purchases/purchases.service.ts`, `credit-products.ts`                |
| payments       | `src/domain/payments`, `src/service/payments`            | provider-neutral 결제 상태·금전 원장·서명된 provider event 처리 | `PaymentsController`, `PaymentsService`                    | database, Polar/Apple/Google adapters                 | `local-payment.provider.spec.ts`, `test/credits.e2e-spec.ts`                               | `src/domain/payments/payments.service.ts`, `payment-provider.ts`                 |
...
| notifications  | `src/domain/notifications`, `src/service/notifications`  | 알림 생성·팔로우 알림 물질화·목록·읽음                          | `NotificationsController`, `NotificationsService`          | database (posts·stories·follows 테이블 직접 읽기)      | `notifications.service.spec.ts`, `test/notifications.e2e-spec.ts`, `test/credits.e2e-spec.ts` | `src/domain/notifications/notifications.service.ts`, `notification-types.ts`  |
...
| 크레딧 예약/캡처   | `CreditsService.reserveCredits`/`captureReservation`/`releaseReservation` | messages(chat_reply) 등 액션           | 멱등·사용자 단위 직렬화·TTL 5분. `expiresAt: null`을 넘기면 소유한 작업이 수명을 관리하는 예약(만료 없음)이 되고, capture/release는 호출자 책임. 다른 쓰기와 한 트랜잭션으로 묶어야 하면 `*WithClient` 변형을 쓴다 | `src/domain/credits/credits.service.ts`                                                             |
| 활성 예약 판정     | `activeReservationFilter`                                                 | credits 잔액, purchases 환불 차단      | `status=reserved` + (만료 없음 or 아직 안 지남). 잔액과 환불이 서로 다른 조건을 쓰면 진행 중인 DM 답변 예약이 한쪽에만 보인다 | `src/domain/credits/credits.service.ts`, `src/domain/purchases/purchases.service.ts`                 |
| DM 답변 작업/워커  | `MessageReplyJob` / `MessageReplyWorker`                                   | `POST /messages`, `POST /messages/retry` | 사용자 메시지 1건 = 작업 1건 = 답변 1건. 대화 단위 advisory lock으로 선점하고 DB lease로 소유. 대화별 직렬·대화 간 병렬, 재시도 3회/처리 시작 후 15분. 자동 tick은 `MESSAGE_REPLY_WORKER_ENABLED=false`로 끄고 `runOnce()`를 직접 부를 수 있다(테스트가 그렇게 한다) | `src/domain/messages/message-reply.worker.ts`                                                        |
...
| 결제 provider 경계 | `PaymentProvider` / `PaymentsService`                                     | purchases, payment webhook             | 구매 로직은 Polar/Apple/Google SDK 타입을 직접 import하지 않음                  | `src/domain/payments/payment-provider.ts`                                                           |
...
- **미해결**: `chat_reply` 원가·마진(측정 필요) → 2크레딧 가격 미확정.
  결제 provider 운영 credential·상품 ID 등록, 성인인증 provider 실연동 방향,
  staging/rollback/backup/monitoring.
