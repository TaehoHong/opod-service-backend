...
| credits        | `src/domain/credits`, `src/service/credits`              | 불변 크레딧 원장·잔액·사용 출처·예약·회수·출석                  | `CreditsController`, `CheckInController`, `CreditsService`    | database                                              | `credits.service.spec.ts`, `payment-refund-coverage.spec.ts`, `test/credits.e2e-spec.ts`, `test/check-in.e2e-spec.ts`      | `src/domain/credits/credits.service.ts`, `src/service/credits/check-in.controller.ts`, `credit-pricing.ts`                 |
| purchases      | `src/domain/purchases`, `src/service/purchases`          | 크레딧 상품·구매·사용자 환불·Polar 누적 부분환불 회수 조정      | `PurchasesController`, `PurchasesService`                     | database, payments, credits, notifications            | `purchases.service.spec.ts`, `test/credits.e2e-spec.ts`                                                                   | `src/domain/purchases/purchases.service.ts`, `credit-products.ts`                |
| payments       | `src/domain/payments`, `src/service/payments`            | provider-neutral 결제 상태·금전 원장·서명된 provider event 처리 | `PaymentsController`, `PaymentsService`                       | database, Polar/Apple/Google adapters                 | `local-payment.provider.spec.ts`, `polar-payment.provider.spec.ts`, `test/credits.e2e-spec.ts`                            | `src/domain/payments/payments.service.ts`, `payment-provider.ts`                 |
