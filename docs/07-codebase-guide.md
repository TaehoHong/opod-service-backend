...
| messages       | `src/domain/messages`, `src/service/messages`            | DM 대화·전송(크레딧 차감, opod-agent 호출)·읽음/미읽음          | `MessagesController`, `MessagesService`                    | database, credits, events, characters, reply-provider | `messages.service.spec.ts`, `message-reply.provider.spec.ts`, `test/messages.e2e-spec.ts` | `src/domain/messages/messages.service.ts`                                        |
