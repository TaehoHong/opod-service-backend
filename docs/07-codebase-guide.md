...
| purchases      | `src/domain/purchases`, `src/service/purchases`          | 크레딧 상품·구매·환불 유스케이스 조정                           | `PurchasesController`, `PurchasesService`                  | database, payments, credits                           | `purchases.service.spec.ts`, `test/credits.e2e-spec.ts`                                    | `src/domain/purchases/purchases.service.ts`, `credit-products.ts`                |
